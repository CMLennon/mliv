...
p_load(
  ...
  car, future, future.apply
)


BelloniData <- function(samplesize = c(1000), muvec = seq(from = 500, to = 5000, by = 100), typeof = "exponential", S = c(5, 50), k = 100) {
  ...
  for (n in samplesize) {
    ...
    if (typeof == "exponential") {
      for (musq in muvec) {
        ...
        #final matrix
        product<-cbind(z,X,Y, C)
        product_list[[paste0('musq_',musq)]] = product
        filename = paste("BelloniData","n",n,"typeof",typeof, 
                         "mu", musq, '.csv', sep = "_")
        print(filename)
        #write_csv(product, paste0('/Users/connor/Desktop/',filename))
...
list_of_mus = BelloniData()
znames = paste(rep("z", 100), c(1:100), sep = "_")
form = as.formula(paste0('Y ~', paste0(znames, collapse = '+')))
...
for (dta in list_of_mus){
  reg = lm(form, data = dta)
  f <- summary(reg)$fstatistic
  ftest <- f[1]
  C_chosen[paste0('musq_', mu)] <- dta$C
  mu_f_corr[paste0('musq_', mu)] = ftest
  mu = mu + 100
}

bind_rows(
  data.frame(mu_f_corr, C_chosen)
) %>% mutate(iter = iter) %>% data.table()
}

# Function: Run iterations ---------------------------------------------------------------
run_sim <-function(n_sims, seed = 12345,future_cores = 12) {
  future::plan("multicore", workers = future_cores)
  # Set the seed
  set.seed(seed)
  # Run a parallelized simulation
  future_lapply(X = 1:n_sims, FUN = oneiter) %>% rbindlist()
}

sim_res = run_sim(n_sims = 40)

write_csv(sim_res, '/Users/connor/Dropbox/MLIV/Plots:Resources/Mu-Fscore-matching_widerange.csv')

my.summary = function(x) list(mean = mean(x, na.rm = T), stddev = sd(x))

cols = sim_res %>% names()
result = sim_res[,unlist(lapply(.SD, my.summary)), .SDcols = cols]
