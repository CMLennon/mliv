...
final_nnet = function(datatable, folds, width, dropout, depth, epoch_num, batch_size, activationfcn, oos = FALSE, foldnum = 5, z_prob = FALSE){
	...
	foldmapper = function(i, folds){
 	...
 	if(z_prob == TRUE){
 	  ...
 	  datatable = datatable %>% mutate(z_prob= ifelse(abs(sin(datatable$z_3 + datatable$z_1 + datatable$z_2)) > .85, round(sin(datatable$z_3 + datatable$z_1 + datatable$z_2)/10,2), 0))
 	...
 	if(oos == TRUE){
 		...
 		for(fold in 1:foldnum){
 			train_dt = datatable %>% filter(fold.id2 != foldnum) %>% data.table() %>% arrange(id)
    		...
    		test_dt = datatable %>% filter(fold.id2 == foldnum) %>% data.table() %>% arrange(id)
		...
		history = model %>% fit(x = train_dat, y =train_lab, epochs = epoch_num, batch_size = batch_size, verbose = 0)
