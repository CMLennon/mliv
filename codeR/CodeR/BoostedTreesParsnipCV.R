...
xgbst_fit = function(seed, datatable, folds, oos = T){
  ...
  search_grid = expand.grid(trees = seq(1,400, by = 100), mtry = seq(1,num_zs, by = round(num_zs/3)), tree_depth = seq(1,20, by = 4))
  
  bst_mod = boost_tree(mode = 'regression', trees = tune(), mtry = tune(), tree_depth = tune()) %>% set_engine('xgboost')
